...
// each plugins/<name>-claude / <name>-codex root: its manifest, grouped by the manifest's
// `name` (matt-loop, auto-loop) so both roots and the marketplace entry must agree.
...
for (const dir of fs.readdirSync(path.join(root, "plugins"))) {
  for (const manifest of [".claude-plugin/plugin.json", ".codex-plugin/plugin.json"]) {
    const file = path.join("plugins", dir, manifest);
    if (!fs.existsSync(path.join(root, file))) continue;
    const m = read(file);
    add(m.name, file, m.version);
  }
}
