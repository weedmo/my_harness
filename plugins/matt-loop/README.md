...
- `agents/` — task-tier routing agents for Claude Code (`matt-loop:matt-default` opus/medium, `matt-deep` fable/high); each fixes model and reasoning effort, so matt-auto, pr-babysit, and resolving-merge-conflicts route by task without an effort picker
