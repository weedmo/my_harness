...
The user settles the big frame in ordinary conversation *before* invoking this skill. By the time matt-auto starts, the remaining questions are implementation-level — the kind where the user would almost always take the recommended answer anyway. So a **decision delegate** (a persistent subagent) answers them, and the human is consulted exactly three ways: the effort pick at the start, immediate escalation of material decisions, and one confirm before tickets publish.
...
2. **Pick effort (human)** — ask the user which reasoning effort the delegate should run at (low / medium / high / xhigh / max; recommend **high**). This is the only upfront human input.
...
5. **Size branch** — fits one session? Present the interview log to the user for confirm (the small path's only gate, since it produces no tickets), then run `$implement` right here (it drives `$tdd`, then `$code-review`, then commits) and finish. Otherwise continue.
...
- Platform without subagents: run the flow human-in-the-loop instead — every sub-skill question goes to the user directly, and step 8's confirm is simply to-tickets' own approval.
...
- Publishing tickets before step 8's confirm → that confirm is the user's only gate over the whole design; don't take it away.
